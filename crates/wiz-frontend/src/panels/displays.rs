    ...
    fn ui(&mut self, ui: &mut egui::Ui) {
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            for (i, display) in self.displays.iter_mut().enumerate() {
                ...
                let header = egui::CollapsingHeader::new(format!("{}: {}", type_name, display.topic))
                    ...
                    .show(ui, |ui| {
                        ...
                        match display.display_type {
                            DisplayType::PointCloud2 => {
                                ui.horizontal(|ui| {
                                    ui.label("Size:");
                                    ui.add(egui::Slider::new(&mut display.point_size, 0.5..=10.0));
                                });
                                ui.horizontal(|ui| {
                                    ui.label("Alpha:");
        ...
        ui.menu_button("+ Add Display", |ui| {
            if ui.button("PointCloud2").clicked() {
                self.displays.push(Display::new(DisplayType::PointCloud2, "/topic".to_string()));
                ui.close_menu();
            }
            if ui.button("LaserScan").clicked() {
                self.displays.push(Display::new(DisplayType::LaserScan, "/topic".to_string()));
                ui.close_menu();
            }
            if ui.button("TF").clicked() {
                self.displays.push(Display::new(DisplayType::TF, "".to_string()));
                ui.close_menu();
            }
            if ui.button("Marker").clicked() {
                self.displays.push(Display::new(DisplayType::Marker, "/topic".to_string()));
                ui.close_menu();
            }
        });
    }
